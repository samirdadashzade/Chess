...
function createSolders() {
	...
	bishop = {
		...
		checkFields: function(event) {
			...
			totalRows.forEach(function(item,index){
				if(item>row){
					if(colums[colum-(item-row)] !== undefined){
						var cell='#'+item+colums[colum-(item-row)];
						topLeft.push(cell);
					}
					if(colums[colum+(item-row)] !== undefined){
						var cell='#'+item+colums[colum+(item-row)];
					...
					if(colums[colum-(row-item)] !== undefined){
						var cell='#'+item+colums[colum-(row-item)];
			...
			removeElements(target,topLeft,possibleMoves);
			removeElements(target,topRight,possibleMoves);
			removeElements(target,bottomLeft,possibleMoves);
			removeElements(target,bottomRight,possibleMoves);
...
function move() {
	...
	$('td').click(function(event){
		...
		if (activeFigure.length > 0 && clickCounter === 1 && $(this).html().length === 0 && moves.indexOf('#' + target.id) > -1) {
			...
			$('td').removeClass('possibleMove');
			activeCounter = 0;
			if($('#' + target.id).attr('data-side') != undefined) {
				$('#' + target.id).attr('data-firstMove',1);
			}
...
function removeElements(target, array,possibleMoves){
	// console.log('before',array);
	// for(var i=0; i<array.length;i++){
	// 	if($(array[i]).attr('data-side')===undefined){
			
	// 	}else{
	// 		if($(target).attr('data-side')===$(array[i]).attr('data-side')){
